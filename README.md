...
The primary difference lies in their operational purpose: Kinesis Data Streams is an ingestion and real-time processing platform requiring custom consumers, while Amazon Data Firehose is a data delivery and transformation engine built to load streaming data directly into storage "sinks" with zero management.Key Differences at a GlanceFeatureAmazon Kinesis Data Streams (KDS)Amazon Data Firehose (ADF)Primary Use CaseReal-time ingestion and custom complex processing.Near-real-time ingestion, delivery, and storage sinking.LatencyReal-time (<200ms; down to 70ms with enhanced fan-out).Near-real-time (buffered from 60 to 900 seconds).Scaling & ManagementManual provisioning via shards (or on-demand autoscaling).Fully managed; scales automatically with zero intervention.Data Retention1 to 365 days (allows data replay).No retention; data is deleted immediately upon delivery.Data ConsumersCustom apps, AWS Lambda, Apache Flink, Spark.Predetermined sinks (S3, Redshift, OpenSearch, Splunk, HTTP).Data TransformationRequires external code or applications to transform data.Built-in format conversions (e.g., JSON to Parquet) or inline Lambda.
...
## AWS Well-Architected tool 
 ...
 - AWS Compute Optimizer simply helps you to identify the optimal AWS resource configurations, such as Amazon Elastic Compute Cloud (EC2) instance types, Amazon Elastic Block Store (EBS) volume configurations, and AWS Lambda function memory sizes.
 - The Max IOPS to GiB Ratio Rule: For Amazon EBS Provisioned IOPS SSD (io1) volumes, AWS enforces a maximum ratio of provisioned IOPS to requested volume size of 50:1. Since each volume in this scenario is exactly 10 GiB, the absolute highest IOPS you can provision for it is calculated as: 10 GiB * 50=500 IOPS
 ...
 - The Amazon Kinesis Client Library (KCL) can also be used to simplify building consumer applications by coordinating distributed processing and ensuring that records associated with the same partition key are processed together in order. This architecture makes Kinesis Data Streams well-suited for high-throughput streaming workloads that require durable storage and ordered message processing.
  <img width="664" height="531" alt="image" src="https://github.com/user-attachments/assets/088248e3-b68e-4660-b547-ed93b009932a" />

 - Amazon Data Firehose is designed only for delivering real-time streaming data to destinations such as data lakes, data stores, and analytics services. It ensures reliable data delivery, but it doesn’t guarantee the order of message delivery and processing.
 - Amazon EMR is a managed cluster platform that simplifies running big data frameworks, such as Apache Hadoop and Apache Spark, on AWS to process and analyze vast amounts of data. By using these frameworks and related open-source projects such as Apache Hive and Apache Pig, you can process data for analytics purposes and business intelligence workloads. Additionally, you can use Amazon EMR to transform and move large amounts of data into and out of other AWS data stores and databases such as Amazon Simple Storage Service (Amazon S3) and Amazon DynamoDB.
 <img width="1000" height="246" alt="image" src="https://github.com/user-attachments/assets/96b98b05-1dab-4e66-b890-82dba2d92df4" />
 ...
 - the multi-attach feature can only be enabled on EBS Provisioned IOPS io2 block express or io1 volumes. In addition, multi-attach won’t offer multi-az resiliency because this feature only allows an EBS volume to be attached on multiple instances within an availability zone.
<img width="673" height="500" alt="image" src="https://github.com/user-attachments/assets/abc115e7-fa6d-428b-bda0-f99f387fd134" />
Where IAM Policies Are AssignedPolicies are JSON documents that define what actions are allowed or denied. You assign (attach) them to identities or resources:IAM Users: Assigned directly to a specific person's account credentials.IAM Groups: Assigned to a collection of users so everyone in the group inherits the same permissions.IAM Roles: Assigned directly to the role to define what anyone assuming that role is allowed to do.AWS Resources (Resource-based policies): Assigned directly to resources like Amazon S3 buckets, SQS queues, or KMS keys to define who can access that specific object.Where IAM Roles Are AssignedRoles do not have long-term credentials (like a password). Instead, they are assumed by entities that need temporary permissions. You assign or associate roles to:AWS Services: Assigned to services like EC2 instances or Lambda functions so they can securely access other AWS resources (e.g., an EC2 instance reading from an S3 bucket).Federated Users: Assigned to external identities coming from Corporate Active Directory, Okta, or Google via SAML/OIDC.Cross-Account Users: Assigned to IAM users in a completely different AWS account who need temporary access to your account.Applications: Assigned to mobile or web apps using Amazon Cognito to grant users temporary AWS access
